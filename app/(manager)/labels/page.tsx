...
export default async function LabelsPage() {
  ...
  // Envíos pendientes de imprimir etiqueta (en preparación)
  const pendientes = await prisma.shipment.count({
    where: { status: "EN_PREPARACION" },
  });

  // Historial de lotes de etiquetas (últimos 30)
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {batch.items.map((item) => (
                      <span
                        key={item.id}
                        className="inline-flex items-center gap-1 text-xs bg-gray-50 border border-gray-200 text-gray-600 px-2 py-0.5 rounded-full"
                      >
